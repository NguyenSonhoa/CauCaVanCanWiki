...
fishing-energy:
  enabled: true
  default-maximum: 200
  success-cost: 1               # chỉ trừ SAU khi nhận cá/rác
  full-recovery-seconds: 86400  # hồi từ 0 -> 100% trong 24 giờ, kể cả offline
  numeric-permission-prefix: "caucavancan.energy."
  permission-maximums:
    vip: {permission: "caucavancan.energy.vip", maximum: 300}
    premium: {permission: "caucavancan.energy.premium", maximum: 500}
...
skill-stamina:
  enabled: true
  default-maximum: 100
  regeneration-per-tick: 1      # hồi thụ động mỗi tick status (core cập nhật 1 lần/giây)
  numeric-permission-prefix: "caucavancan.stamina."
  permission-maximums:
    vip: {permission: "caucavancan.stamina.vip", maximum: 150}
    premium: {permission: "caucavancan.stamina.premium", maximum: 250}

# Đây là tài nguyên skill CCVC, không phải Năng lượng câu cá hay MMOCore stamina.
`,
  fishHealth: `
# config.yml
fish-health:
...
const sections = [
  ...
  {
    id: "souls-energy", title: "Linh Hồn, Năng lượng và Stamina", icon: "sparkles", keywords: "soul souls linh hon soul fishing rod craft energy nang luong stamina the luc recovery 24h capacity permission vault mmoitems", desc: "Linh Hồn, Năng lượng câu cá và Stamina skill CCVC là ba tài nguyên riêng; Năng lượng câu cá hồi đầy trong 24 giờ mặc định.",
    html: `<div class="content-grid"><div class="doc-card"><h3>Luồng Linh Hồn</h3>${table(["Trường hợp", "Cách roll"], [["Rác/cá thường", "<code>pools.&lt;pool-id&gt;.regular</code> roll sau khi người chơi thực sự nhận reward. Chọn được rác qua <code>include-trash</code>; cá qua <code>eligible-rarities</code> như SMALL/MEDIUM."],["Cá độc quyền", "<code>pools.&lt;pool-id&gt;.exclusive.&lt;fish-id&gt;</code> có chance và soul riêng. Nó là nhóm riêng, không bị large-rarity-bonus/bonus LARGE thay đổi."],["Hồ premium", "ID pool là ID trong <code>premium-pools.yml</code>; có thể khai báo regular LARGE/VERY_LARGE và exclusive riêng cho hồ premium."]])}<p>Core chỉ roll Linh Hồn sau khi item cá/rác MMOItems được tạo và đưa vào inventory hoặc rơi do đầy túi. Mỗi Soul được core đóng dấu PDC ID; lúc craft chỉ tiêu đúng item Linh Hồn đã đóng dấu, không thể dùng item trùng tên từ plugin khác.</p></div><div class="doc-card half"><h3>Config Linh Hồn</h3>${codeBlock("souls.yml", snippets.soul, "yaml")}<p><code>chance</code> là số thập phân: <code>0.20 = 20%</code>. Map <code>souls</code> trong <code>regular</code> là trọng số chọn <strong>một</strong> Soul sau khi chance đã trúng. ID pool/fish phải khớp chính xác file spawn/premium/fish.</p></div><div class="doc-card half"><h3>Craft Cần câu Linh Hồn</h3><p>Mỗi craft cần một loại Soul (<code>soul-amount: 20</code> theo ví dụ) và có thể kèm <code>money</code>. Đặt <code>money: 0</code> để chỉ cần Soul. Tiền dùng Vault economy; nếu <code>money &gt; 0</code> nhưng Vault/economy không sẵn sàng thì core không trừ Soul và không craft.</p>${callout("warn", "Tạo MMOItems Soul và MMOItems cần đầu ra trước. Copy type <code>SOUL</code> từ <code>mmoitems/item-types.yml</code> vào MMOItems server rồi restart; nếu server dùng type khác, sửa <code>mmoitems.type</code> trong souls.yml.")}</div><div class="doc-card half"><h3>Năng lượng câu cá</h3>${codeBlock("config.yml", snippets.energy, "yaml")}<p>Năng lượng là số nguyên lưu trong SQLite CCVC, hoàn toàn tách với Stamina skill CCVC và stamina MMOCore. Người mới có đầy <code>default-maximum</code> (mặc định 200). Timer hồi từ 0 lên 100% trong 24 giờ, vẫn chạy offline và được lưu SQLite. Core kiểm tra đủ năng lượng trước khi bắt đầu một lượt, nhưng chỉ trừ <code>success-cost</code> sau khi <strong>nhận được cá hoặc rác</strong>; fail/đứt dây/huỷ không trừ.</p>${callout("", "Placeholder: <code>%caucavancan_fishing_energy_recovery%</code> (ví dụ <code>24h</code>, <code>2h 15m</code>, <code>Đầy</code>), <code>%caucavancan_fishing_energy_recovery_seconds%</code> và <code>%caucavancan_fishing_energy_recovery_total%</code>.")}</div><div class="doc-card half"><h3>Stamina skill CCVC</h3>${codeBlock("config.yml", snippets.skillStamina, "yaml")}<p>Đây là thanh dùng để trả <code>stamina-cost</code> khi cast skill. Nó hồi theo <code>regeneration-per-tick</code>, độc lập với Năng lượng câu cá và không đọc MMOCore.</p></div><div class="doc-card"><h3>Lệnh</h3>${table(["Lệnh", "Quyền", "Tác dụng"], [["<code>/fish energy</code>", "Mọi người chơi", "Xem Năng lượng hiện tại/tối đa."],["<code>/fish energy set|add &lt;player&gt; &lt;amount&gt;</code>", "<code>caucavancan.energy.admin</code>", "Đặt/cộng Năng lượng, tự clamp về 0 đến maximum."],["<code>/fish soul</code>", "Mọi người chơi", "Xem tiến độ các công thức."],["<code>/fish soul craft &lt;craft-id&gt;</code>", "<code>caucavancan.soul.craft</code>", "Tiêu Soul đã đóng dấu và Vault money (nếu có) để craft."],["<code>/fish givesoul &lt;player&gt; &lt;soul-id&gt; &lt;amount&gt;</code>", "<code>caucavancan.soul.admin</code>", "Cấp Soul để setup/test."]])}${callout("", "Node trực tiếp như <code>caucavancan.energy.300</code> và tier tên riêng đều hợp lệ. Nếu player có nhiều node, CCVC dùng capacity cao nhất, không cộng các mức với nhau.")}</div></div>`
  },
  {
  ...
  {
    id: "commands", title: "Lệnh và quyền", icon: "terminal", keywords: "commands permission fish att skill energy soul linh hon admin premium reload givebait boost", desc: "Lệnh người chơi, lệnh admin và permission node có trong plugin.yml.",
    html: `<div class="content-grid"><div class="doc-card"><h3>Lệnh người chơi</h3>${table(["Lệnh", "Tác dụng"], [["<code>/fish</code> hoặc <code>/fishing</code>", "Command root; các subcommand dưới đây."],["<code>/fish box</code>", "Mở hộp mồi."],["<code>/fish sell</code>", "Mở menu bán cá."],["<code>/fish chest</code>", "Mở menu chính."],["<code>/fish skills</code>", "Mở skill loadout."],["<code>/fish collection</code> / <code>index</code>", "Mở bộ sưu tập cá."],["<code>/fish boost</code>", "Mở boost menu và báo trạng thái boost."],["<code>/fish energy</code>", "Xem Năng lượng câu cá riêng của CCVC."],["<code>/fish soul</code>", "Xem tiến độ craft Linh Hồn."],["<code>/fish soul craft &lt;id&gt;</code>", "Craft Cần câu Linh Hồn khi đủ Soul/tiền."],["<code>/att</code>", "Mở thuộc tính/dialog."],["<code>/att reset</code>", "Reset attribute và hoàn điểm theo phí cấu hình."],["<code>/skill</code> / <code>loadout</code>", "Mở skill loadout."],["<code>/skill tree</code> / <code>learn</code>", "Mở skill tree."]])}</div><div class="doc-card"><h3>Lệnh admin</h3>${table(["Lệnh", "Quyền", "Ghi chú"], [["<code>/fish reload</code>", "<code>caucavancan.admin</code>", "Nạp lại cấu hình core."],["<code>/fish givebait [player] &lt;id&gt; &lt;amount&gt;</code>", "admin", "Cấp mồi ID trong bait.yml."],["<code>/fish givesoul &lt;player&gt; &lt;id&gt; &lt;amount&gt;</code>", "<code>caucavancan.soul.admin</code>", "Cấp Soul đã đóng dấu để test/setup."],["<code>/fish energy set|add &lt;player&gt; &lt;amount&gt;</code>", "<code>caucavancan.energy.admin</code>", "Sửa Năng lượng câu cá, không phải MMOCore stamina."],["<code>/fish shadowtest ...</code>", "admin", "Thử scale bóng cá."],["<code>/fish boost[global] &lt;type&gt; &lt;percent&gt; &lt;seconds&gt;</code>", "admin", "EXP/GOLD/WEAKEN/POWER/EXCLUSIVE."],["<code>/fish character setlevel|addexp|addatt|addskill &lt;player&gt; &lt;amount&gt;</code>", "<code>caucavancan.character.admin</code>", "Sửa character data online."],["<code>/fish skill points give|set|remove &lt;player&gt; &lt;amount&gt;</code>", "character.admin", "Sửa skill points."],["<code>/fish skill unlock &lt;player&gt; &lt;skill-id&gt; [level]</code>", "character.admin", "Bypass requirement/cost."],["<code>/fish premium grant &lt;player&gt; &lt;seconds&gt;</code>", "<code>caucavancan.premium.admin</code>", "Cấp ticket premium."],["<code>/fish premium revoke &lt;player&gt;</code>", "premium.admin", "Thu ticket premium."],["<code>/fish categorystat &lt;category&gt;</code>", "admin", "Tạo MMOItems category damage stats."]])}</div><div class="doc-card">${pills(["caucavancan.admin", "caucavancan.character.admin", "caucavancan.premium", "caucavancan.premium.admin", "caucavancan.energy.admin", "caucavancan.soul.craft", "caucavancan.soul.admin"], "warn")}<p><code>caucavancan.premium</code> là permission nhập hồ premium mặc định; từng pool có thể đặt node riêng trong <code>premium-pools.yml</code>.</p></div></div>`
  },
  {
...
// 03/09/2026: this page is the canonical guide for the compact lake schema.
sections.push({
  id: "lake-schema-20260903",
  title: "Cập nhật hồ 03/09",
  icon: "refresh-cw",
  keywords: "lakes spawns souls tier mmoitems collection sell premium reel migration",
  desc: "Schema hồ rút gọn, tier tại spawn, mảnh theo hồ và các sửa lỗi battle/GUI mới nhất.",
  html: `<div class="content-grid"><div class="doc-card"><h3>File chính thức</h3>${codeBlock("CauCaVanCanCore", "lakes/\\n  spawns.yml       # world -> region -> normal-fish/exclusive/trash\\n  souls.yml        # soul -> lakes -> normal/exclusive\\nfish/categories/   # nhận diện cá, MMOItems và override ngoại lệ\\ngui/fish-collection.yml\\ngui/sell-menu.yml", "text")}<p><strong>Không dùng lại</strong> <code>fish/spawns.yml</code>. Root <code>souls.yml</code> chỉ được đọc như fallback khi <code>lakes/souls.yml</code> chưa có map <code>souls</code> để không làm mất cấu hình legacy đang chạy.</p></div><div class="doc-card"><h3>Schema spawn mới</h3>${codeBlock("lakes/spawns.yml", "worlds:\\n  world:\\n    region:\\n      ho_bo_hoang:\\n        required-level: 1\\n        normal-fish:\\n          fish:\\n            CA_CHEP_DO_DOT_BIEN: {chance: 27, tier: SMALL}\\n            CA_LIEN_DUNG_DI_BIEN: {chance: 21, tier: MEDIUM}\\n        exclusive:\\n          chance: 0.01\\n          fish: {VUONG_CA_CHEP: 1}\\n        trash:\\n          chance: 0.15\\n          items: {old_boot: 50, rusty_can: 50}", "yaml")}<p>Normal và trash phải tổng 100. Cá trong <code>exclusive.fish</code> là tier <code>EXCLUSIVE</code> và dùng trọng số, không cần tổng 100.</p></div><div class="doc-card half"><h3>Tier, bóng và file cá</h3><p>Tier do spawn quyết định, nên một loài có thể có tier khác ở mỗi hồ. <code>fish-shadows.categories.&lt;TIER&gt;</code> quản lý <code>display-name</code>, <code>shadow-size</code>, <code>glow-color</code>. File cá không còn cần default <code>health</code>, <code>pull-per-second</code>, <code>max-safe-distance</code>, <code>line-penalty-per-second</code>, <code>rarity</code>, <code>required-level</code>, <code>shadow-size</code> hay <code>mmocore-exp</code>; chỉ ghi chúng khi là ngoại lệ riêng của loài.</p></div><div class="doc-card half"><h3>lakes/souls.yml</h3>${codeBlock("lakes/souls.yml", "souls:\\n  LINH_HON_XUXIAN:\\n    display-name: \\\"&dLinh Hồn Xuxian\\\"\\n    mmoitems: {type: SOUL, id: LINH_HON_XUXIAN}\\n    lakes:\\n      ho_bo_hoang:\\n        normal: {chance: 0.05, amount: 1}\\n        exclusive: {chance: 0.20, amount: 1}", "yaml")}<p>Mỗi soul khai báo MMOItems một lần. <code>normal</code> và <code>exclusive</code> là chance riêng theo từng hồ.</p></div><div class="doc-card half"><h3>Collection và menu bán</h3><p>Collection lấy item MMOItems thật (thiếu definition sẽ hiện <code>BARRIER</code>); chỉ OP thấy lore ID. Menu bán có 5 hàng input, chỉ hàng cuối là kính action bar. Nút bán cập nhật số cá hợp lệ và tổng <strong>“Bạn sẽ nhận: X vàng”</strong> khi click/kéo item.</p></div><div class="doc-card half"><h3>Battle và premium</h3><p>Thu cần vanilla (<code>REEL_IN</code>) hủy phiên để không treo battle. Cá kéo người chơi, spit đã tăng tầm/trúng đích, và ghì dây ghi nhận quest <code>WIN_TUG</code>. Dùng <code>/fish premium grant &lt;player&gt; &lt;seconds&gt;</code> để cấp vé; lệnh báo số giây thực cấp.</p></div><div class="doc-card"><h3>Triển khai an toàn</h3><ol><li>Build/test JAR trước.</li><li>Merge ba phía cho YAML: giữ mọi key live đã tùy biến, thêm default mới.</li><li>Backup remote, upload tên tạm, thay thế atomically và tải lại để verify.</li><li>Restart server sau khi thay JAR; dùng <code>/fish reload</code> sau khi chỉ sửa YAML hợp lệ.</li></ol></div></div>`
});
